Attribute VB_Name = "PipelineDashboard"
Option Explicit

' ============================================================
' PASTE YOUR DASHBOARD FILE PATH HERE (include the filename):
'   Example: "C:\Users\You\Documents\Pipeline_Cost_Dashboard.html"
...
'
'   RefreshPipelineDashboard  -> "Refresh Data" button
'        Reads the "Finalized Projects" sheet and rewrites
'        Pipeline_Cost_Dashboard.html with the latest ALL_DATA.
'
'   OpenPipelineDashboard     -> "Open Dashboard" button
'        Opens the HTML file in your default browser.
'
' Path priority:
'   1. HTML_PATH_OVERRIDE constant above (if set)
'   2. Dashboard sheet cell B4
'   3. Workbook folder + Dashboard sheet cell B2 (filename)
' ============================================================

' ── COMBINED: refresh then open (assign to "Refresh & Open" button) ──────────
Sub RefreshAndOpenPipelineDashboard()
    RefreshPipelineDashboard
    OpenPipelineDashboard
End Sub

' ── REFRESH ONLY ─────────────────────────────────────────────────────────────
Sub RefreshPipelineDashboard()
    ...
    If htmlPath = "" Then
        MsgBox "No HTML path found." & vbCrLf & vbCrLf & _
               "Please paste the full path to Pipeline_Cost_Dashboard.html" & vbCrLf & _
    ...
    If LCase(Right(Trim(htmlPath), 5)) <> ".html" And _
       LCase(Right(Trim(htmlPath), 4)) <> ".htm" Then
        MsgBox "SAFETY STOP – path does not end in .html:" & vbCrLf & htmlPath & vbCrLf & vbCrLf & _
               "Check that cell B4 on the Dashboard sheet contains the HTML file path.", _
    ...
    If LCase(Trim(htmlPath)) = LCase(ThisWorkbook.FullName) Then
        MsgBox "SAFETY STOP – path points to this workbook!" & vbCrLf & _
               "Update cell B4 on the Dashboard sheet with the path to the HTML dashboard.", _
    ...
    If Dir(htmlPath) = "" Then
        MsgBox "HTML file not found at:" & vbCrLf & htmlPath & vbCrLf & vbCrLf & _
               "Update cell B4 on the Dashboard sheet with the correct path.", _
    ...
    For i = 2 To lastRow
        ...
        ' ── Parse Section from File Path (col A) ─────────────────────────────
        ' File Path starts with e.g. "Sec. 100 Monthly Analysis..." or "Sec. 200..."
        Dim vSec As String
        Dim filePath As String
        filePath = Trim(CStr(wsData.Cells(i, 1).Value))
        vSec = ""
        Dim secPos As Long
        secPos = InStr(1, filePath, "Sec. ", vbTextCompare)
        If secPos > 0 Then
            Dim spaceAfter As Long
            spaceAfter = InStr(secPos + 5, filePath, " ")
            If spaceAfter > 0 Then
                vSec = Mid(filePath, secPos, spaceAfter - secPos)
            Else
                vSec = Mid(filePath, secPos)
            End If
        End If
...
SkipRow:
    ...
    If startPos = 0 Then
        MsgBox "ALL_DATA marker not found in the HTML file." & vbCrLf & _
               "Is this the correct Pipeline_Cost_Dashboard.html?" & vbCrLf & vbCrLf & htmlPath, _
...
' ── Path resolver ─────────────────────────────────────────────────────────────
' Priority: 1) HTML_PATH_OVERRIDE  2) Dashboard!B4  3) workbook folder + Dashboard!B2
Private Function GetHtmlPath() As String
    ...
    ' 2. Dashboard sheet B4 (full path) and B2 (filename)
    ...
    If Not dashWs Is Nothing Then
        On Error Resume Next
        p = Trim(CStr(dashWs.Cells(4, 2).Value))   ' B4 = full path
        On Error GoTo 0
        ...
        Dim fName As String
        On Error Resume Next
        fName = Trim(CStr(dashWs.Cells(2, 2).Value))   ' B2 = filename
        On Error GoTo 0
    ...
    If ThisWorkbook.Path <> "" Then
        GetHtmlPath = ThisWorkbook.Path & "\Pipeline_Cost_Dashboard.html"
    End If
End Function
