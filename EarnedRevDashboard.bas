...
'
'   RefreshEarnedRevDashboard  -> "Refresh Data" button
'        Reads Sheet1 and rewrites Earned_Revenue_Dashboard.html
'        with the latest ER data. Does NOT open the browser.
'
'   OpenEarnedRevDashboard     -> "Open Dashboard" button
'        Opens the HTML file in your default browser.
'
'   RefreshAndOpenEarnedRev    -> calls both in sequence
'
' Path priority:
...
Sub RefreshEarnedRevDashboard()
    ...
    ' Col 10 = J = Jan 2026  ...  Col 57 = BE = Dec 2029
    ...
    For col = FIRST_MONTH_COL To LAST_MONTH_COL
        ...
        ' Normalize: anything containing "actual" → "Actual", else "Forecast"
    ...
    ' ── Build projects JSON (rows 4 to 25001, cols A-BE) ─────────────────────
    ' Column mapping (Sheet1):
    '   A(1)  = Project Number (code)
    '   B(2)  = Client
    '   C(3)  = Status ("Secured" / "Anticipated")
    ...
    '   H(8)  = End Date
    '   I(9)  = Duration (months)
    '   J(10) - BE(57) = monthly[0..47] earned revenue values
    ...
    For i = 4 To 25001
        ...
        ' Read 48 monthly values (cols 10–57)
        ...
        ' Build startIdx / endIdx (null if no non-zero values)
        ...
        ' Build record (split across lines to stay under VBA 24-continuation limit)
        Dim rec As String
        rec = "{"
...
SkipRow:
    ...
    ' Find the next 'const ' declaration after ER to mark the end of the block
    ...
    ' Replace: everything from startPos up to (not including) the newline before next const
...
' ── OPEN ─────────────────────────────────────────────────────────────────────
Sub OpenEarnedRevDashboard()
    Dim htmlPath As String
    htmlPath = GetHtmlPath()
    If htmlPath = "" Or Dir(htmlPath) = "" Then
        MsgBox "HTML file not found." & vbCrLf & _
               "Check that cell B3 on the Dashboard sheet has the correct path.", _
...
' ── REFRESH + OPEN ───────────────────────────────────────────────────────────
Sub RefreshAndOpenEarnedRev()
    RefreshEarnedRevDashboard
    OpenEarnedRevDashboard
End Sub
...
Private Function GetHtmlPath() As String
    ...
    ' 3. Same folder as workbook + default filename
...
' Converts a Double to a JSON number string (dot decimal, no thousand separators)
